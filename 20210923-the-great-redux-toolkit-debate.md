...
1. **It's unopinionated.** Redux requires you to put your global state in a store, and to manage that state via reducers and actions. An action is a simple JavaScript object with a `type` field, and a reducer is a pure function that transforms the old state into the new state based on an action. Beyond this, everything else is up to you.
...
3. **It's extremely versatile.** Do you want your store to contain only the ID of the current user? Or do you want your store to track the state of every entity, page, widget, and input in your massive enterprise app? Whatever you use case, Redux and its large ecosystem have you covered.
...
2. **It has a minimal API surface.** You'll quickly realize you need more than just `createStore` to create a useful application with Redux. A prime example of this is the need to fetch data from an API in response to an action. _(More on this later)._
...
- `createAsyncThunk` — allows you to start API calls in response to an action and dispatch another action when the API succeeds or fails. Thinking about thunks hurts my brain but I'm sure I could learn them given some time.
- `createEntityAdapter` — returns a set of prebuilt reducers and selector functions for performing CRUD on an entity.
- RTK Query — a **library** for fetching and caching server state in your Redux store. Can be compared to [React Query](https://react-query.tanstack.com/) which aims to solve the same problems in a different way.
...
By including `createAsyncThunk` in Redux Toolkit, the Redux team has made thunks the officially-recommended side effect model for Redux. I like how Redux itself is unopinionated regarding side effects, and so I have mixed feelings about the built-in support for thunks.
...
RTK Query seems complex and unintuitive to me, but my opinion might change if I tried it out. To be fair, I've never been a proponent of storing all of your server state in the Redux store, unless you need to support offline. If you're looking for an alternative, you might like [React Query](https://react-query.tanstack.com/). I also evaluated the similar [SWR](https://swr.vercel.app/) library but found it lacking some features that my team uses constantly.
...
- There's a dash of Redux Saga to support user login and persisting changes to complex order drafts that the user creates.
...
This app has to work offline so it made sense to put majority of the app's state in Redux.

- Redux Saga is responsible for all the interaction with the backend API and this worked out quite well. There's a pretty complex saga for sending queued operations to the backend when a user comes back from being offline.
...
- Local component state is still used for forms.

### My Next React Web App

New projects are always exciting because they give you the chance to rethink your entire architecture and tech stack. Going forward, we'll be:

- Making a difficult decision between continuing to use typesafe-actions and `switch` reducers, or switching to Redux Toolkit's `createAction` and `createReducer`.
...
## The Section where I Self-Promote
